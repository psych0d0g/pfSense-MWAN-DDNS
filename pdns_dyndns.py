...
def get_public_ipv4_addresses():
    ...
    for line in output.splitlines():
        if line and not line.startswith("\t") and ":" in line:
            ...
            if allowed_physical_interfaces and iface not in allowed_physical_interfaces:
                continue
            ...
            if match:
                ...
                if ip.startswith("10.") or ip.startswith("192.168."):
                    continue
                if 172 <= int(octets[0]) <= 172 and 16 <= int(octets[1]) <= 31:
                    continue
...
def get_gateway_interface_map():
    ...
    try:
        ...
        for gw_item in root.findall(".//gateways/gateway_item"):
            ...
            if gw_name and interface:
                mapping[gw_name] = interface
    except Exception as e:
        print(f"❌ Could not parse gateway to interface map: {e}")
    return mapping

def get_pfsense_interface_mapping():
    mapping = {}
    try:
        with open("/cf/conf/config.xml", "r") as f:
            config = f.read()
        tree = ET.fromstring(config)
        for iface in tree.findall(".//interfaces/*"):
            if iface.tag in ["lan", "wan"] or iface.tag.startswith("opt"):
                pf_iface_name = iface.tag
                physical_iface_name = iface.findtext("if")
                if physical_iface_name:
                    mapping[physical_iface_name] = pf_iface_name
    except Exception:
        pass
    return mapping

def map_ips_to_interfaces():
    result = subprocess.run(["/sbin/ifconfig"], stdout=subprocess.PIPE, text=True)
    output = result.stdout
    ip_to_iface = {}
    iface = None
    for line in output.splitlines():
        if line and not line.startswith("\t") and ":" in line:
            ...
            if match4:
                ip_to_iface[match4.group(1)] = iface
            match6 = re.search(r"inet6 ([a-f0-9:]+)", line)
            if match6:
                ip = match6.group(1).split("%")[0]
                ip_to_iface[ip] = iface
    return ip_to_iface

def get_pfsense_dyndns_ids():
    mapping = {}
    try:
        tree = ET.parse('/conf/config.xml')
        root = tree.getroot()
        for dyndns_entry in root.findall(".//dyndnses/dyndns"):
            if dyndns_entry.findtext("type") == "custom":
                ...
                if interface and entry_id:
                    mapping[interface] = entry_id
    except Exception:
        pass
    return mapping


# === State Handling & DNS Update Functions ===
def send_push_notification(subject, message):
    safe_message = message.replace('"', '\\"').replace("`", "'")
    php_code = f"""
    require_once("/etc/inc/notices.inc");
    file_notice("dynupdate", "{safe_message}", "DynDNS", "", 1, false);
    """
    try:
        subprocess.run(["/usr/local/bin/php", "-r", php_code], check=True)
    except Exception as e:
        print("Push-Nachricht konnte nicht gesendet werden:", e)
...
def save_state(ipv4, ipv6):
    timestamp = datetime.utcnow().isoformat()
    state = {
        "ipv4": {ip: timestamp for ip in ipv4},
        "ipv6": {ip: timestamp for ip in ipv6},
    }
    with open(state_file, "w") as f:
        json.dump(state, f)

def update_dns(ipv4_addresses, ipv6_addresses):
    rrsets = []
    rrsets.append({"name": record_name, "type": "A", "ttl": ttl, "changetype": "REPLACE", "records": [{"content": ip, "disabled": False} for ip in ipv4_addresses]})
    rrsets.append({"name": record_name, "type": "AAAA", "ttl": ttl, "changetype": "REPLACE", "records": [{"content": ip, "disabled": False} for ip in ipv6_addresses]})
    try:
        zone_url = f"{api_url}/servers/{server_id}/zones/{zone}"
        data = json.dumps({"rrsets": rrsets}).encode("utf-8")
        req = urllib.request.Request(zone_url, data=data, headers={"X-API-Key": api_key, "Content-Type": "application/json"}, method="PATCH")
...
def update_cache_files(healthy_ipv4, unhealthy_ipv4, healthy_ipv6, unhealthy_ipv6, mappings):
    """Updates pfSense cache files to reflect the health status of each IP."""
    ...
    all_ips_to_process = {
        'healthy': healthy_ipv4 + healthy_ipv6,
        'unhealthy': list(unhealthy_ipv4) + list(unhealthy_ipv6)
    }
    ...
    for status, ip_list in all_ips_to_process.items():
        for ip in ip_list:
            ...
            # Write without newline for healthy (green), with newline for unhealthy (red)
...
if __name__ == "__main__":
    print(f"--- DynDNS script started at {datetime.now().isoformat()} (Reason: {args.reason}) ---")

    # 1. Get all system mappings and configs
    thresholds = get_gateway_monitoring_thresholds()
    gateway_statuses = get_gateway_statuses(thresholds)
    gateway_to_if_map = get_gateway_interface_map()
    phys_to_pf_if_map = get_pfsense_interface_mapping()
    ip_to_phys_if_map = map_ips_to_interfaces()
    if_to_gateway_map = {v: k for k, v in gateway_to_if_map.items()}
    dyndns_id_map = get_pfsense_dyndns_ids()

    print(f"Gateway Thresholds: {thresholds}")
    print(f"Gateway Statuses: {gateway_statuses}")

    # 2. Get all public IPs from all interfaces
    all_ipv4 = get_public_ipv4_addresses()
    all_ipv6 = get_public_ipv6_addresses()
    ...
    for ip in all_ipv4:
        ...
        gw_status = gateway_statuses.get(gw_name, 'unknown')
        if gw_status == 'online':
    ...
    if args.ipv4only: healthy_ipv6, unhealthy_ipv6 = [], set()
    if args.ipv6only: healthy_ipv4, unhealthy_ipv4 = [], set()
    ...
    if args.force_update or ipv4_changed or ipv6_changed:
        if not args.force_update: print("Change detected, performing DNS update...")
        else: print(f"Forcing DNS update (Reason: {args.reason})...")

        if update_dns(healthy_ipv4, healthy_ipv6):
            save_state(healthy_ipv4, healthy_ipv6)

            mappings = {'ip_to_phys': ip_to_phys_if_map, 'phys_to_pf': phys_to_pf_if_map, 'dyndns_ids': dyndns_id_map}
            update_cache_files(healthy_ipv4, unhealthy_ipv4, healthy_ipv6, unhealthy_ipv6, mappings)

            print("DNS update and cache files successful.")
            msg = f"DynDNS for {record_name} updated.\nHealthy IPs:\nIPv4: {healthy_ipv4}\nIPv6: {healthy_ipv6}"
            send_push_notification("DynDNS Gateway Update", msg)
        else:
